...
if [ ! -f "${SITE_PATH}/settings.php" ]; then
    ...
    drush site:install standard \
      ...
      --site-name="Simple API Banner Demo" \
      --account-name=admin \
      --account-pass=admin \
      -y
...
drush php:eval "use \Drupal\block\Entity\Block; Block::create(['id' => 'claro_simpleapibanner', 'theme' => 'claro', 'region' => 'highlighted', 'plugin' => 'simple_api_banner_block', 'settings' => ['label' => 'Simple API Banner', 'label_display' => 'visible'], 'visibility' => ['request_path' => ['id' => 'request_path', 'negate' => false, 'pages' => '<front>'], 'response_status' => ['id' => 'response_status', 'negate' => false, 'status_codes' => [200]]]])->save();"
drush php:eval "use \Drupal\block\Entity\Block; Block::create(['id' => 'olivero_simpleapibanner', 'theme' => 'olivero', 'region' => 'highlighted', 'plugin' => 'simple_api_banner_block', 'settings' => ['label' => 'Simple API Banner', 'label_display' => 'visible'], 'visibility' => ['request_path' => ['id' => 'request_path', 'negate' => false, 'pages' => '<front>'], 'response_status' => ['id' => 'response_status', 'negate' => false, 'status_codes' => [200]]]])->save();"
drush php:eval "\$config = \Drupal::configFactory()->getEditable('simple_api_banner.settings'); \$config->set('endpoint_url', 'http://host.docker.internal:8080/simple_api_banner_data.php')->set('response_json_text_path', 'content')->set('response_json_author_path', 'author')->set('cache_duration', '1 hour')->save();"
